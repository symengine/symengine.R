...
static inline
SEXP wrap_basic_func_onearg(SEXP x,
    ...
    if (Rf_isS4(x)) {
        sexp_check_basic(R_do_slot(x, Rf_mkString(".xData")));
        a = (basic_struct*) R_ExternalPtrAddr(R_do_slot(x, Rf_mkString(".xData")));
        s = (basic_struct*) R_ExternalPtrAddr(R_do_slot(out, Rf_mkString(".xData")));
    }
    else {
        // Is a externalptr
        sexp_check_basic(x);
        a = (basic_struct*) R_ExternalPtrAddr(x);
        s = (basic_struct*) R_ExternalPtrAddr(out);
    }
