...
test_that("S(integer) works", {
    x <- S(42L)
    expect_identical(type(x), "Integer")
})
...
test_that("S(double) enabled check_whole_number", {
    ...
    x <- S(42.1)
    expect_identical(type(x), "RealDouble")
})
...
test_that("S(character) can be parsed as constant", {
    ...
    x <- S("e")
    expect_identical(type(x), "Constant")
})
...
test_that("S(formula) works", {
    x <- S(~ x)
    expect_true(type(x) == "Symbol")
    
    x <- S(~ e)
    expect_true(type(x) == "Symbol")
    expect_true(type(x) != type(S("e")))
    
    x <- S(~ 3)
    expect_true(type(x) == "Integer")
    
    x <- S(~ 3.1)
    expect_true(type(x) == "RealDouble")
